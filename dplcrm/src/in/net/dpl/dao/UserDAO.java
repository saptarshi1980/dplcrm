...
public int resetPassword(String user_id,String newpassword){	
	...
	public User userLoginAuth(String user_id,String password){  
		User user= new User();
		user.setUserName("U");
		String sql = "SELECT DISTINCT a.party_code,b.meter_no,a.name,a.email_id,a.mob_no FROM consdb.m_party a,consdb.x_bill b,user_master_online c WHERE c.user_id='"+user_id+"' AND AES_DECRYPT(c.pass,'saptarshi')='"+password+"' AND c.con_no=a.PARTY_CODE AND a.party_code=b.party_code";
		List<Map<String, Object>> rows = jdbcTemplate.queryForList(sql);
		int counter=0;
