...
public class PaymentRequestDAO {
	...
	public List<PaymentRequest> billList(String conNo){  
		String sql = "SELECT a.party_code,DATE_FORMAT(STR_TO_DATE(a.bill_month,'%y%m'),'%M-%Y') AS bill_month_word,a.bill_month,a.bill_amt,a.rebate,DATE_FORMAT(a.due_date,'%d-%m-%Y') AS due_date,b.name FROM v_bill_info a,consdb.v_party_info b WHERE a.party_code='"+conNo+"' AND  a.party_code=b.party_code AND a.due_date>=CURDATE() AND a.bill_month NOT IN (SELECT a.bill_month FROM v_bill_info a,transaction b WHERE b.consumer_no='"+conNo+"' AND b.consumer_no=a.party_code AND DATE_FORMAT(STR_TO_DATE(a.bill_month,'%y%m'),'%Y-%m')=DATE_FORMAT(b.bill_month,'%Y-%m') AND b.billdesk_status='0300' ) ";
