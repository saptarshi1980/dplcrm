...
<a href="https://thedpl.in/docupload">Home</a>
...
<center>
        ...
        <c:forEach items="${categories.rowsByIndex}" var="row" begin="${start}" end="${stop}">
            <tr>
                <c:forEach begin="0" end="${columns}" var="x">
                    ...
                    <c:choose>
                    <c:when test="${x=='0'}"><td><a href="GetTariffFiles.dpl?params=${row[x]}">Download Papers</td></a></c:when>
    ...
    <c:forEach begin="1" end="${numberOfPages}" var="i">
        <c:choose>
            <c:when test="${i!=pageNumber}">
                <a href="displayTariff.dpl?pageNumber=<c:out value="${i}"/>"><c:out value="${i}"/></a>
            </c:when>
            <c:otherwise>
                <c:out value="${i}"/>
            </c:otherwise>        
    ...
    <c:if test="${pageNumber lt numberOfPages}">
        <a href="displayTariff.dpl?pageNumber=${pageNumber + 1}">Next</a>
    </c:if>
</center>
